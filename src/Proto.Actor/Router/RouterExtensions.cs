...
public static class RouterExtensions
{
    ...
    /// <summary>
    /// Creates props for a router, that routes the messages to the routees by calculating the hash of the message key and finding a routee on a hash ring.
    ...
    /// <summary>
    /// Creates props for a router, that routes the messages to the routees by calculating the hash of the message key and finding a routee on a hash ring.
    /// If the message is <see cref="IHashable"/>, then the key extracted with <see cref="IHashable.HashBy"/> takes precedence. Otherwise, the hash key
    ...
    /// <summary>
    /// Creates props for a router, that routes the messages to the pool of actors it maintains by calculating the hash of the message key and finding a pool member on a hash ring.
    /// If the message is <see cref="IHashable"/>, then the key extracted with <see cref="IHashable.HashBy"/> takes precedence. Otherwise, the hash key
    /// is extracted from the message with a provided delegate.
    /// </summary>
    /// <param name="senderContext">Context to send the messages through</param>
