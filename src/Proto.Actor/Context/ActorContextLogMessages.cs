...
{
    [LoggerMessage(0, LogLevel.Debug, "{Self} Responding to {Sender} with message {MessagePayload}")]
    internal static partial void ActorResponds(this ILogger logger, PID self, PID sender, object? message);

    [LoggerMessage(1, LogLevel.Warning, "{Self} Tried to respond but sender is null, with message {MessagePayload}")]
    ...
    [LoggerMessage(4, LogLevel.Warning, "SystemMessage cannot be forwarded. {MessagePayload}")]
    internal static partial void SystemMessageCannotBeForwarded(this ILogger logger, object? message);

    [LoggerMessage(5, LogLevel.Error, "[Supervision] Actor {Self} : {ActorType} failed with message:{MessagePayload}")]
    ...
    [LoggerMessage(6, LogLevel.Error, "Error handling SystemMessage {MessagePayload}")]
    internal static partial void ErrorHandlingSystemMessage(this ILogger logger, Exception ex, object? message);

    [LoggerMessage(7, LogLevel.Warning, "Unknown system message {MessagePayload}")]
    internal static partial void UnknownSystemMessage(this ILogger logger, object? message);

    [LoggerMessage(8, LogLevel.Warning, "{Self} Dropping Continuation (ReenterAfter) of {MessagePayload}")]
