//-----------------------------------------------------------------------
// <copyright file="Source.cs" company="Proto.NET Project">
...
namespace Proto.Streams.Dsl
{
    ...
    public sealed class Source<TOut, TMat> : IFlow<TOut, TMat>, IGraph<SourceShape<TOut>, TMat>
    {
        ...
        /// <summary>
        /// Use the `ask` pattern to send a request-reply message to the target <paramref name="pid"/>.
        ...
        /// </summary>
        public Source<TOut2, TMat> Ask<TOut2>(PID pid, TimeSpan timeout, int parallelism = 2)
        {
            ...
            var askFlow = Flow.Create<TOut>()
                ...
                .SelectAsync(parallelism, async e => {
                    var reply = await pid.Ask(e, timeout: timeout);
                    switch (reply)
                    {
    ...
    public static class Source
    {
        ...
        /// <summary>
        /// Creates a <see cref="Source{TOut,TMat}"/> that is materialized to an <see cref="PID"/> which points to an Actor
        ...
        /// </exception>
        /// <returns>TBD</returns>
        public static Source<T, PID> ActorPublisher<T>(Props props)
        {
            if (!typeof(Actors.ActorPublisher<T>).IsAssignableFrom(props.Type))
                throw new ArgumentException("Actor must be ActorPublisher");

            return new Source<T, PID>(new ActorPublisherSource<T>(props, DefaultAttributes.ActorPublisherSource, Shape<T>("ActorPublisherSource")));
        }

        /// <summary>
        /// Creates a <see cref="Source{TOut,TMat}"/> that is materialized as an <see cref="PID"/>.
        ...
        /// </exception>
        /// <returns>TBD</returns>
        public static Source<T, PID> ActorRef<T>(int bufferSize, OverflowStrategy overflowStrategy)
        {
            ...
            return new Source<T, PID>(new ActorRefSource<T>(bufferSize, overflowStrategy, DefaultAttributes.ActorRefSource, Shape<T>("ActorRefSource")));
        }
