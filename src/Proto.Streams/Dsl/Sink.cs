//-----------------------------------------------------------------------
// <copyright file="Sink.cs" company="Proto.NET Project">
...
namespace Proto.Streams.Dsl
{
    ...
    public static class Sink
    {
        ...
        ///<summary>
        /// Sends the elements of the stream to the given <see cref="PID"/>.
        ...
        /// <returns>TBD</returns>
        public static Sink<TIn, NotUsed> ActorRef<TIn>(PID pid, object onCompleteMessage)
            => new Sink<TIn, NotUsed>(new ActorRefSink<TIn>(pid, onCompleteMessage, DefaultAttributes.ActorRefSink, Shape<TIn>("ActorRefSink")));

        /// <summary>
        /// Sends the elements of the stream to the given <see cref="PID"/> that sends back back-pressure signal.
        ...
        /// <returns>TBD</returns>
        public static Sink<TIn, NotUsed> ActorRefWithAck<TIn>(PID pid, object onInitMessage, object ackMessage,
            object onCompleteMessage, Func<Exception, object> onFailureMessage = null)
        {
            ...
            return
                FromGraph(new ActorRefBackpressureSinkStage<TIn>(pid, onInitMessage, ackMessage,
                    onCompleteMessage, onFailureMessage));
        }

        ///<summary>
        /// Creates a <see cref="Sink{TIn,TMat}"/> that is materialized to an <see cref="PID"/> which points to an Actor
        ...
        /// <returns>TBD</returns>
        public static Sink<TIn, PID> ActorSubscriber<TIn>(Props props)
            => new Sink<TIn, PID>(new ActorSubscriberSink<TIn>(props, DefaultAttributes.ActorSubscriberSink, Shape<TIn>("ActorSubscriberSink")));
