//-----------------------------------------------------------------------
// <copyright file="IOSources.cs" company="Proto.NET Project">
...
namespace Proto.Streams.Implementation.IO
{
    ...
    internal sealed class FileSource : SourceModule<ByteString, Task<IOResult>>
    {
        ...
        public override IPublisher<ByteString> Create(MaterializationContext context, out Task<IOResult> task)
        {
            ...
            var pid = materializer.ActorOf(context, props.WithDispatcher(dispatcher.Name));

            task = ioResultPromise.Task;
            return new ActorPublisherImpl<ByteString>(pid);
            ...
            try
            {
                ...
                var props = InputStreamPublisher
                    ...
                    .WithDispatcher(context
                        ...
                        .Name);
                var pid = materializer.ActorOf(context, props);
                pub = new ActorPublisherImpl<ByteString>(pid);
            }
            catch (Exception ex)
            {
