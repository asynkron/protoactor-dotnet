//-----------------------------------------------------------------------
// <copyright file="TcpStages.cs" company="Proto.NET Project">
...
using StreamTcp = Proto.Streams.Dsl.Tcp;
using Tcp = Proto.IO.Tcp;

namespace Proto.Streams.Implementation.IO
{
    /// <summary>
    /// INTERNAL API
    /// </summary>
    internal sealed class ConnectionSourceStage : GraphStageWithMaterializedValue<SourceShape<StreamTcp.IncomingConnection>, Task<StreamTcp.ServerBinding>>
    {
        ...
        private sealed class ConnectionSourceStageLogic : TimerGraphStageLogic, IOutHandler
        {
            ...
            private StreamTcp.IncomingConnection ConnectionFor(Tcp.Connected connected, PID connection)
            {
            ...
            private void Receive((PID, object) args)
            {
        ...
        private readonly PID _tcpManager;
        ...
        public ConnectionSourceStage(PID tcpManager, EndPoint endpoint, int backlog,
    ...
    {
        private readonly PID _connection;
        ...
        /// <param name="halfClose">TBD</param>
        public IncomingConnectionStage(PID connection, EndPoint remoteAddress, bool halfClose)
        {
    ...
    internal static class TcpConnectionStage
    {
        ...
        internal struct Outbound : ITcpRole
        {
            ...
            /// <param name="halfClose">TBD</param>
            public Outbound(PID manager, Tcp.Connect connectCmd, TaskCompletionSource<EndPoint> localAddressPromise, bool halfClose)
            {
                Manager = manager;
        ...
        internal struct Inbound : ITcpRole
        {
            ...
            /// <param name="connection">TBD</param>
            /// <param name="halfClose">TBD</param>
            public Inbound(PID connection, bool halfClose)
            {
            ...
            /// TBD
            /// </summary>
            public readonly PID Connection;
        ...
        internal sealed class TcpStreamLogic : GraphStageLogic
        {
            ...
            private void Connected((PID, object) args)
            {
                var msg = args.Item2;
        ...
        GraphStageWithMaterializedValue<FlowShape<ByteString, ByteString>, Task<StreamTcp.OutgoingConnection>>
    {
        ...
        public OutgoingConnectionStage(PID tcpManager, EndPoint remoteAddress, EndPoint localAddress = null,
