//-----------------------------------------------------------------------
// <copyright file="ActorRefBackpressureSinkStage.cs" company="Proto.NET Project">
...
namespace Proto.Streams.Implementation
{
    ...
    internal class ActorRefBackpressureSinkStage<TIn> : GraphStage<SinkShape<TIn>>
    {
        ...
        private sealed class Logic : InGraphStageLogic
        {
            ...
            public PID Self => StageActor.Ref;
            ...
            {
                _stage._pid.Tell(_stage._onFailureMessage(ex), Self);
                _completionSignalled = true;
                FailStage(ex);
            }

            private void Receive((PID, object) evt)
            {
                var msg = evt.Item2;
                ...
                if (t != null && Equals(t.ActorRef, _stage._pid))
                    CompleteStage();
            ...
            {
                SetKeepGoing(true);
                GetStageActor(Receive).Watch(_stage._pid);
                _stage._pid.Tell(_stage._onInitMessage, Self);
                Pull(_stage._inlet);
            }
            ...
            private void Finish()
            {
                _stage._pid.Tell(_stage._onCompleteMessage, Self);
                _completionSignalled = true;
                CompleteStage();
            }
        ...
        public ActorRefBackpressureSinkStage(PID pid, object onInitMessage, object ackMessage,
