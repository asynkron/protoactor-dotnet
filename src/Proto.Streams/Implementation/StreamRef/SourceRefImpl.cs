//-----------------------------------------------------------------------
// <copyright file="SourceRefImpl.cs" company="Proto.NET Project">
...
namespace Proto.Streams.Implementation.StreamRef
{
    ...
    {
        public static SourceRefImpl Create(Type eventType, PID initialPartnerRef)
        {
        ...
        protected SourceRefImpl(PID initialPartnerRef)
        {
            InitialPartnerRef = initialPartnerRef;
        }

        public PID InitialPartnerRef { get; }
    ...
    {
        public SourceRefImpl(PID initialPartnerRef) : base(initialPartnerRef) { }
    ...
    internal sealed class SourceRefStageImpl<TOut> : GraphStageWithMaterializedValue<SourceShape<TOut>, Task<ISinkRef<TOut>>>
    {
        ...
        private sealed class Logic : TimerGraphStageLogic, IOutHandler
        {
            ...
            public PID Self => _stageActor.Ref;
            public PID PartnerRef
            {
                get
                {
            ...
            private void InitialReceive((PID, object) args)
            {
                ...
                switch (message)
                {
                    ...
                    case Terminated terminated:
                        if (Equals(_partnerRef, terminated.Who))
                            ...
                            FailStage(new RemoteStreamRefActorTerminatedException(
                                $"Received UNEXPECTED Terminated({terminated.Who}) message! This actor was NOT our trusted remote partner, which was: {_partnerRef}. Tearing down."));
            ...
            private void ObserveAndValidateSender(PID partner, string failureMessage)
            {
                if (_partnerRef == null)
                {
        ...
        private readonly PID _initialPartnerRef;

        public SourceRefStageImpl(PID initialPartnerRef)
        {
