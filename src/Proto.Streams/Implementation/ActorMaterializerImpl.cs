//-----------------------------------------------------------------------
// <copyright file="ActorMaterializerImpl.cs" company="Proto.NET Project">
...
namespace Proto.Streams.Implementation
{
    ...
    public abstract class ExtendedActorMaterializer : ActorMaterializer
    {
        ...
        public abstract TMat Materialize<TMat>(IGraph<ClosedShape, TMat> runnable, Func<GraphInterpreterShell, PID> subFlowFuser);
        ...
        public abstract TMat Materialize<TMat>(IGraph<ClosedShape, TMat> runnable, Func<GraphInterpreterShell, PID> subFlowFuser, Attributes initialAttributes);
        ...
        public override PID ActorOf(MaterializationContext context, Props props)
        {
        ...
        protected PID ActorOf(Props props, string name, string dispatcher)
        {
            ...
            if (repointableActorRef != null)
            {
                ...
                var f = repointableActorRef.Ask<PID>(new StreamSupervisor.Materialize(props.WithDispatcher(dispatcher), name), timeout);
                return f.Result;
            }
    ...
    public sealed class ActorMaterializerImpl : ExtendedActorMaterializer
    {
        ...
        private sealed class ActorMaterializerSession : MaterializerSession
        {
            ...
            private readonly Func<GraphInterpreterShell, PID> _subflowFuser;
            private readonly string _flowName;
            private int _nextId;

            public ActorMaterializerSession(ActorMaterializerImpl materializer, IModule topLevel, Attributes initialAttributes, Func<GraphInterpreterShell, PID> subflowFuser)
        ...
        private readonly PID _supervisor;
        ...
        /// <returns>TBD</returns>
        public ActorMaterializerImpl(ActorSystem system, ActorMaterializerSettings settings, Dispatchers dispatchers, PID supervisor, AtomicBoolean haveShutDown, EnumerableActorName flowNames)
        {
            _system = system;
            _settings = settings;
        ...
        public override PID Supervisor => _supervisor;
        ...
        /// <param name="subFlowFuser">TBD</param>
        /// <returns>TBD</returns>
        public override TMat Materialize<TMat>(IGraph<ClosedShape, TMat> runnable, Func<GraphInterpreterShell, PID> subFlowFuser) 
        ...
        /// <exception cref="IllegalStateException">TBD</exception>
        /// <returns>TBD</returns>
        public override TMat Materialize<TMat>(IGraph<ClosedShape, TMat> runnable, Func<GraphInterpreterShell, PID> subFlowFuser, Attributes initialAttributes)
        {
    ...
    public class SubFusingActorMaterializerImpl : IMaterializer
    {
        ...
        private readonly Func<GraphInterpreterShell, PID> _registerShell;
        ...
        public SubFusingActorMaterializerImpl(ExtendedActorMaterializer delegateMaterializer, Func<GraphInterpreterShell, PID> registerShell)
        {
    ...
    public class StreamSupervisor : ActorBase
    {
        ...
        public sealed class Children
        {
            ...
            /// TBD
            /// </summary>
            public readonly IImmutableSet<PID> Refs;
            /// <summary>
            /// TBD
            /// </summary>
            /// <param name="refs">TBD</param>
            public Children(IImmutableSet<PID> refs)
            {
                Refs = refs;
            }
        }
