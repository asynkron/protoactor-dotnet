//-----------------------------------------------------------------------
// <copyright file="GraphStage.cs" company="Proto.NET Project">
...
namespace Proto.Streams.Stage
{
        ...
        protected StageActor GetStageActor(StageActorRef.Receive receive)
        {
            if (_stageActor == null)
            {
                ...
                _stageActor = new StageActor(
                    actorMaterializer,
                    r => GetAsyncCallback<(PID, object)>(message => r(message)),
                    receive,
                    StageActorName);
            }
            else
    ...
    {
        public delegate void Receive((PID, object) args);
    }
    ...
    {
        private readonly Action<(PID, object)> _callback;
        ...
        public StageActor(
            ...
            Func<StageActorRef.Receive, Action<(PID, object)>> getAsyncCallback,
        ...
        /// <summary>
        /// The <see cref="PID"/> by which this <see cref="StageActor"/> can be contacted from the outside.
        /// This is a full-fledged <see cref="PID"/> that supports watching and being watched
        /// as well as location transparent (remote) communication.
        /// </summary>
        public PID Ref => _functionRef;
        ...
        /// <summary>
        /// Makes current <see cref="StageActor"/> watch over given <paramref name="pid"/>.
        /// It will be notified when an underlying actor is <see cref="Terminated"/>.
        /// </summary>
        /// <param name="pid"></param>
        public void Watch(PID pid) => _functionRef.Watch(pid);

        /// <summary>
        /// Makes current <see cref="StageActor"/> stop watching previously <see cref="Watch"/>ed <paramref name="pid"/>.
        /// If <paramref name="pid"/> was not watched over, this method has no result.
        /// </summary>
        /// <param name="pid"></param>
        public void Unwatch(PID pid) => _functionRef.Unwatch(pid);

        internal void InternalReceive((PID, object) pack)
        {
