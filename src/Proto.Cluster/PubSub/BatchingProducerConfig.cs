...
public record BatchingProducerConfig
{
    ...
    /// <summary>
    /// Max size of the requests waiting in queue. If value is provided, the producer will throw <see cref="ProducerQueueFullException"/> when queue size is exceeded. If null, the queue is unbounded. Default: null.
    /// </summary>
    ...
    /// <summary>
    /// Error handler that can decide what to do with an error when publishing a batch. Default: Fail and stop the <see cref="BatchingProducer"/>
    /// </summary>
    public PublishingErrorHandler OnPublishingError { get; set; } = (_, _, _) => Task.FromResult(PublishingErrorDecision.FailBatchAndStop);

    /// <summary>
    /// A throttle for logging from this producer. By default, a throttle shared between all instances of <see cref="BatchingProducer"/> is used, that allows for 10 events in 10 seconds.
    /// </summary>
