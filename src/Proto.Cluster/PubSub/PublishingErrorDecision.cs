...
{
    internal TimeSpan? Delay { get; private set; } = null;
    ...
    /// <summary>
    /// Skips the current batch and proceeds to the next one. The delivery reports (tasks) related to that batch are still failed with the exception that triggered the error handling.
    /// </summary>
    ...
    /// <returns></returns>
    public static PublishingErrorDecision RetryBatchAfter(TimeSpan delay) => new()
    {
        Delay = delay
    };
