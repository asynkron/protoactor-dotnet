...
invoke `MemberList.UpdateClusterTopology` with the current set of members【F:src/Proto.Cluster.Kubernetes/KubernetesClusterMonitor.cs†L304-L317】.
...
`Left`, and `Blocked` lists【F:src/Proto.Cluster/Member/MemberList.cs†L161-L240】.
The resulting topology is published to the node's local event stream via
`BroadcastTopologyChanges`【F:src/Proto.Cluster/Member/MemberList.cs†L349-L353】.
...
for inclusion in the gossip state【F:src/Proto.Cluster/Gossip/Gossiper.cs†L190-L204】.
The gossip implementation stores the full membership under the `cluster:topology`
key and tracks active member IDs for later consensus checks【F:src/Proto.Cluster/Gossip/Gossip.cs†L85-L92】.

## Gossip dissemination

The gossip loop periodically updates heartbeat information and sends the current
state to randomly chosen peers【F:src/Proto.Cluster/Gossip/Gossiper.cs†L232-L241】【F:src/Proto.Cluster/Gossip/Gossip.cs†L168-L205】.
...
included in the published topology【F:src/Proto.Cluster/Member/MemberList.cs†L197-L238】.
...
provider【F:src/Proto.Cluster/Cluster.cs†L286-L299】. Other nodes read this key and
block those members【F:src/Proto.Cluster/Gossip/Gossiper.cs†L262-L277】.
...
entries cause nodes to be added to the block list【F:src/Proto.Cluster/Gossip/Gossiper.cs†L281-L310】.
